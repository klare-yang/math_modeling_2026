# code/m03_fit_m3_judges_bayes.py
import argparse
import json
...
import pymc as pm
import arviz as az
import pytensor.tensor as pt  # <-- FIX: for tensor constants


DEFAULT_PANEL = Path("data/M01_panel_m3.csv")
DEFAULT_MAP = Path("data/M01_keymap_m3.json")

DEFAULT_TRACE_OUT = Path("data/M03_trace_m3_judges.nc")
DEFAULT_SUMMARY_OUT = Path("data/M03_summary_m3_judges.csv")
DEFAULT_DIAG_OUT = Path("data/M03_diagnostics_m3_judges.json")


def main():
    parser = argparse.ArgumentParser(
        description="M03: Fit Bayesian LMM for JUDGES outcome (score_z) with pro/season/celeb random effects."
    )
    ...
    if not panel_path.exists():
        raise FileNotFoundError(f"[M03] Panel not found: {panel_path}")
    if not map_path.exists():
        raise FileNotFoundError(f"[M03] Map not found: {map_path}")
    ...
    if missing:
        raise ValueError(f"[M03] Missing required columns in panel: {missing}")
    ...
    # -----------------------------
    print("\n" + "=" * 90)
    print("M03 CHECK BLOCK START")
    print("=" * 90)
    print(f"[M03] panel_path = {panel_path}")
    print(f"[M03] map_path   = {map_path}")
    print(f"[M03] include_celeb_re = {bool(args.include_celeb_re)}")
    print(f"[M03] rows_before_drop = {before}")
    print(f"[M03] rows_after_drop  = {len(panel2)}")
    print(f"[M03] dropped_missing_required = {dropped}")

    print("\n[M03] DIMENSIONS (from meta):")
    ...
    assert pro_id.max() < len(pro_names), "[M03] pro_id out of range."
    assert industry_id.max() < len(ind_names), "[M03] industry_id out of range."
    assert season_id.max() < len(season_vals), "[M03] season_id out of range."
    assert celeb_id.max() < len(celeb_names), "[M03] celeb_id out of range."
    ...
    with pm.Model(coords=coords) as model:
        ...
        if args.include_celeb_re:
            ...
            # FIX: use tensor constants, not python floats
            sigma_celeb = pm.Deterministic("sigma_celeb", pt.as_tensor_variable(0.0))
            a_celeb = pm.Deterministic("a_celeb", pt.zeros((len(celeb_names),)), dims="celeb")
        ...
        ppc = pm.sample_posterior_predictive(idata, var_names=["likelihood"], random_seed=args.seed, progressbar=True)
        idata.extend(ppc)

    print(f"[M03] Saving trace to: {out_trace}")
    idata.to_netcdf(out_trace, engine="h5netcdf")

    summary_vars = ["beta0", "beta_age", "beta_week", "sigma_eps", "sigma_pro", "sigma_season", "sigma_celeb"]
    summ = az.summary(idata, var_names=summary_vars, round_to=6)
    summ.to_csv(out_summary)
    print(f"[M03] Saved summary: {out_summary}")
    ...
    diag: Dict[str, Any] = {
        ...
        "qc": {
            ...
            "max_id_check": max_ok,
            "include_celeb_re": bool(args.include_celeb_re),
            "sigma_celeb_fixed_if_disabled": 0.0 if not args.include_celeb_re else None,
        },
    }

    with open(out_diag, "w", encoding="utf-8") as f:
        json.dump(diag, f, ensure_ascii=False, indent=2)
    print(f"[M03] Saved diagnostics: {out_diag}")

    print("\n" + "=" * 90)
    print("M03 POST-SAMPLING CHECK BLOCK START")
    print("=" * 90)
    print("[M03] Summary (key vars):")
    print(summ.to_string())
    print("\n[M03] Diagnostics:")
    print(json.dumps(diag["qc"], indent=2))
    print("M03 POST-SAMPLING CHECK BLOCK END")
    print("=" * 90 + "\n")
