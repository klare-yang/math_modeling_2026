...
DEFAULT_TRACE_OUT = Path("data/M06_trace_m3_fans_celeb.nc")
DEFAULT_SUMMARY_OUT = Path("data/M06_summary_m3_fans_celeb.csv")
DEFAULT_DIAG_OUT = Path("data/M06_diagnostics_m3_fans_celeb.json")


def main():
    parser = argparse.ArgumentParser(
        description="M06: Fit Bayesian LMM for FAN outcome (logit share) with pro/season/celeb random effects (celeb RE ON by default)."
    )
    ...
    if not panel_path.exists():
        raise FileNotFoundError(f"[M06] Panel not found: {panel_path}")
    if not map_path.exists():
        raise FileNotFoundError(f"[M06] Map not found: {map_path}")
    ...
    if missing:
        raise ValueError(f"[M06] Missing required columns in panel: {missing}")
    ...
    # -----------------------------
    print("\n" + "=" * 90)
    print("M06 CHECK BLOCK START")
    print("=" * 90)
    print(f"[M06] panel_path = {panel_path}")
    print(f"[M06] map_path   = {map_path}")
    print(f"[M06] include_celeb_re = {bool(args.include_celeb_re)}")
    print(f"[M06] rows_before_drop = {before}")
    print(f"[M06] rows_after_drop  = {len(panel2)}")
    print(f"[M06] dropped_missing_required = {dropped}")

    print("\n[M06] DIMENSIONS (from meta):")
    ...
    assert pro_id.max() < len(pro_names), "[M06] pro_id out of range."
    assert industry_id.max() < len(ind_names), "[M06] industry_id out of range."
    assert season_id.max() < len(season_vals), "[M06] season_id out of range."
    assert celeb_id.max() < len(celeb_names), "[M06] celeb_id out of range."
    ...
    with pm.Model(coords=coords) as model:
        ...
        # Residual SD (latent structural noise)
    ...
    print(f"[M06] Saving trace to: {out_trace}")
    ...
    print(f"[M06] Saved summary: {out_summary}")
    ...
    print(f"[M06] Saved diagnostics: {out_diag}")

    print("\n" + "=" * 90)
    print("M06 POST-SAMPLING CHECK BLOCK START")
    print("=" * 90)
    print("[M06] Summary (key vars):")
    print(summ.to_string())
    print("\n[M06] Diagnostics:")
    print(json.dumps(qc, indent=2))
    print("M06 POST-SAMPLING CHECK BLOCK END")
    print("=" * 90 + "\n")
